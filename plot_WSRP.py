...
# Set up for Tide/Gyre
   ...
   DATA_DIR = os.path.join('/gpfs/'+WSRP_DIR[6]+'p2/ptmp/Logan.Dawson/WSRP', ymdh, 'data')
   GRAPHX_DIR = os.path.join('/gpfs/'+WSRP_DIR[6]+'p2/ptmp/Logan.Dawson/WSRP', ymdh, 'graphx')
...
try:
   ...
   if resolution == '0.25':
      print 'plotting '+resolution+'-deg data'
...
grib1file = DATA_DIR+'/gfsanl.'+ymdh[0:8]+'.t'+ymdh[8:]+'z.pgrb.'+resolution[0]+'p'+resolution[2:].ljust(2,'0')+'.f000.grib'
grib2file = DATA_DIR+'/gfsanl.'+ymdh[0:8]+'.t'+ymdh[8:]+'z.pgrb2.'+resolution[0]+'p'+resolution[2:].ljust(2,'0')+'.f000.grib2'
...
def plot_levels(level):
   ...
   elif level == '700' or level == '850' or level == 'sfc':
      domain = 'EastNA'
   ...
   if level == '250' or level == '300':
    ...
    # colorlist = ['lightsteelblue','skyblue','dodgerblue','lightpink','hotpink']
  ...
  #   else:
      ...
      msg = "Upward vertical velocity ($\omega$) \nshown in dashed contours \nevery -5 $\mathregular{\mu}$$\mathregular{b}$ $\mathregular{s^{-1}}$"
      ax.annotate(msg,xy=(0.885,-0.1),xycoords='axes fraction',color='k',zorder=12,fontsize=10,ha='center', va='center',bbox=dict(fc="white"))
   ...
   fname = level+'plot_'+YYYYMMDDCC+'_'+resolution[0]+'p'+resolution[2:].ljust(2,'0')+'deg.png'
...
#for j in range(1):
   ...
   print 'plotting '+YYYYMMDDCC+' analysis'

   grbs = pygrib.open(DATA_DIR+'/gfsanl.'+YYYYMMDDCC[0:8]+'.t'+YYYYMMDDCC[8:]+'z.'+grib_mod+'.'+resolution[0]+'p'+resolution[2:].ljust(2,'0')+'.f000.'+grib_ext)
...
#  print omega_700.min(), omega_700.max()
   ...
   if resolution != '2.0':
      hghts_250 = scipy.ndimage.gaussian_filter(hghts_250,sigma)
 #    isotach_250 = scipy.ndimage.gaussian_filter(isotach_250,sigma)

      hghts_300 = scipy.ndimage.gaussian_filter(hghts_300,sigma)
 #    isotach_300 = scipy.ndimage.gaussian_filter(isotach_300,sigma)

      hghts_500 = scipy.ndimage.gaussian_filter(hghts_500,sigma)
 #    isotach_400 = scipy.ndimage.gaussian_filter(isotach_400,sigma)
      vort_500 = scipy.ndimage.gaussian_filter(vort_500,sigma3)

      hghts_700 = scipy.ndimage.gaussian_filter(hghts_700,sigma)
      omega_700 = scipy.ndimage.gaussian_filter(omega_700,sigma)
 #    rh_700 = scipy.ndimage.gaussian_filter(rh_700,sigma)

      hghts_850 = scipy.ndimage.gaussian_filter(hghts_850,sigma2)
 #    isotach_850 = scipy.ndimage.gaussian_filter(isotach_850,sigma)
      uwind_850 = scipy.ndimage.gaussian_filter(uwind_850,sigma)
      vwind_850 = scipy.ndimage.gaussian_filter(vwind_850,sigma)
      temp_850 = scipy.ndimage.gaussian_filter(temp_850,sigma2)

      if MSLET or resolution == '1.0':
         mslp = scipy.ndimage.gaussian_filter(mslp,sigma2)
      thick1000_500 = scipy.ndimage.gaussian_filter(thick1000_500,sigma)
